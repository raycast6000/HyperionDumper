...
int dump(const char* what) {
	...
	if (!ReadProcessMemory(robloxProcess, (void*)(modBase + dosHeader.e_lfanew), &ntHeaders, sizeof(IMAGE_NT_HEADERS), &nBytes)) {
	...
	for (DWORD64 address = modBase; address < (modBase + imageSize); address++) {
		...
		ReadProcessMemory(robloxProcess, (void*)address, &b, sizeof(BYTE), 0);

		// printf("0x%llx: Read 0x%x\n", address, b);
		data[dBytesRead] = b;
		dBytesRead += 1;
	}

	dBytesRead = 0;
	printf("(%s) Checking signature: 0x", what);
	for (; dBytesRead < 2; dBytesRead++) {
		printf("%x", data[dBytesRead]);
	}

	DWORD lol = (DWORD)data;
	printf("\n(%s) Amount of read bytes: 0x%llx\n", what, strlen((char*)data));
